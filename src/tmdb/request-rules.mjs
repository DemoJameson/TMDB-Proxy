...
async function applyTmdbRequestRules(request, options = {}) {
	...
	if (isTmdbCompatiblePath(url) && !url.searchParams.get("api_key") && !hasAuthorization) url.searchParams.set("api_key", DEFAULT_TMDB_API_KEY);
	if (isForwardHost(url.hostname)) {
		...
		if (needsRedirect) {
			if (!tmdbUrl.searchParams.get("api_key")) tmdbUrl.searchParams.set("api_key", DEFAULT_TMDB_API_KEY);
...
export { applyTmdbRequestRules, DEFAULT_TMDB_API_KEY, encodeState, fetchTmdbWithNativeFetch, fetchUpstream, STATE_HEADER };
